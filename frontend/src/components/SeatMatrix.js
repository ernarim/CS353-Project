...
export default function SeatMatrix({rows, columns, available_seats, onSeatClick}){
    const seatMatrix = []
    ...
    const disableSeats = (arr) => {
        ...
        seatMatrix.forEach( (row) => {
            row.forEach((seat) => {
                if(curr_available < arr.length && arr[curr_available][0] == seat[0] && arr[curr_available][1] == seat[1]){
                    curr_available++;
                }
                else{
                    seat[2] = false;
            ...
            <Row key={i} style={{marginBottom: 5}} gutter={[8, 8]}>
                {row.map((seat, j) => {
                    if(!seat[2]){
                        return(
                            <Col key={j}>
                                <Seat number={seat[0] + "-" + seat[1]} isActive={true}/>
                            </Col>
                        )
                    } else {
                        return(
